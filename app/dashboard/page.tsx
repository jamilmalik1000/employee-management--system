...
const stats = [
  { label: "Total Employees", value: "124", change: "+4 this month",  trend: "up",   icon: "👨💼", color: "#6366f1", bg: "rgba(99,102,241,0.08)",  border: "rgba(99,102,241,0.15)" },
  { label: "Departments",     value: "9",   change: "No change",      trend: "flat", icon: "🏢",   color: "#d97706", bg: "rgba(217,119,6,0.08)",   border: "rgba(217,119,6,0.15)"  },
...
const recentActivity = [
  { icon: "👤", text: "New employee Ali Hassan joined Engineering",      time: "2 min ago",  color: "#6366f1", bg: "rgba(99,102,241,0.08)"  },
  { icon: "📅", text: "Sara Khan submitted a leave request for 3 days", time: "18 min ago", color: "#d97706", bg: "rgba(217,119,6,0.08)"   },
  { icon: "✏️", text: 'Role "HR Manager" permissions were updated',     time: "1 hr ago",   color: "#7c3aed", bg: "rgba(124,58,237,0.08)"  },
  { icon: "✅", text: "Leave request approved for Omar Farooq",         time: "3 hrs ago",  color: "#059669", bg: "rgba(5,150,105,0.08)"   },
  { icon: "🔐", text: "New user account created for Ayesha Malik",      time: "Yesterday",  color: "#2563eb", bg: "rgba(37,99,235,0.08)"   },
  { icon: "🏢", text: 'Department "Finance" headcount updated to 14',   time: "Yesterday",  color: "#d97706", bg: "rgba(217,119,6,0.08)"   },
];

const departments = [
  { name: "Engineering", count: 28, color: "#6366f1", pct: 23 },
  { name: "Operations",  count: 22, color: "#2563eb", pct: 18 },
  { name: "Marketing",   count: 18, color: "#059669", pct: 15 },
  { name: "Finance",     count: 14, color: "#d97706", pct: 11 },
  { name: "HR",          count: 12, color: "#7c3aed", pct: 10 },
  { name: "Others",      count: 30, color: "#94a3b8", pct: 23 },
];
...
export default function DashboardPage() {
  ...
  return (
    <div className="page-root">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "1rem" }}>
        {stats.map((s) => (
          <div key={s.label} className="card" style={{ padding: "1.25rem 1.5rem" }}>
            ...
            <div style={{ marginTop: "0.875rem", display: "flex", alignItems: "center", gap: "0.375rem", color: s.trend === "up" ? "#dc2626" : s.trend === "down" ? "#059669" : "#94a3b8", fontSize: "0.75rem", fontWeight: 600 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "1rem", alignItems: "start" }} className="dashboard-grid">
        ...
        <div style={{ background: "#fff", borderRadius: "1rem", border: "1px solid #e8ecf4", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", overflow: "hidden" }}>
          ...
          <div style={{ padding: "0.5rem 0" }}>
            {recentActivity.map((a, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "0.875rem", padding: "0.875rem 1.5rem", borderBottom: i < recentActivity.length - 1 ? "1px solid #f8faff" : "none" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: "0.875rem", color: "#1e293b", fontWeight: 500, margin: 0, lineHeight: 1.4 }}>{a.text}</p>
                  <p style={{ fontSize: "0.75rem", color: "#94a3b8", margin: "0.25rem 0 0" }}>{a.time}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ background: "#fff", borderRadius: "1rem", border: "1px solid #e8ecf4", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", overflow: "hidden" }}>
          <div style={{ padding: "1.125rem 1.5rem", borderBottom: "1px solid #f0f2f8", background: "#fafbff" }}>
            <h2 style={{ fontSize: "0.9375rem", fontWeight: 700, color: "#1e293b", margin: 0 }}>Departments</h2>
          </div>
          <div style={{ padding: "1rem 1.5rem", display: "flex", flexDirection: "column", gap: "0.875rem" }}>
            {departments.map((d) => (
              <div key={d.name}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.375rem" }}>
                  <span style={{ fontSize: "0.8125rem", fontWeight: 600, color: "#334155" }}>{d.name}</span>
                  <span style={{ fontSize: "0.8125rem", color: "#64748b", fontWeight: 600 }}>{d.count}</span>
                </div>
