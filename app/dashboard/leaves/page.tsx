    ...
    <div className="page-root">
      ...
      <div className="card" style={{ overflow: "hidden" }}>
        {loading ? <div style={{ padding: "4rem", textAlign: "center", color: "#94a3b8" }}><Loader2 className="animate-spin" style={{ margin: "auto" }} />Loading leave requests...</div> : visible.length === 0 ? <div style={{ padding: "4rem", textAlign: "center", color: "#94a3b8" }}><CalendarDays size={42} style={{ margin: "0 auto .75rem" }} />No leave requests found.</div> :
          <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: 760 }}><thead><tr style={{ background: "#f8fafc" }}>{["Employee", "Type", "Dates", "Reason", "Status", ...(management ? ["Actions"] : [])].map((head) => <th key={head} style={{ padding: ".9rem 1rem", textAlign: "left", fontSize: ".7rem", color: "#64748b", textTransform: "uppercase" }}>{head}</th>)}</tr></thead><tbody>{visible.map((leave) => <tr key={leave.id} style={{ borderTop: "1px solid #eef2f7" }}><td style={{ padding: "1rem", fontWeight: 700 }}>{leave.employeeName}</td><td style={{ padding: "1rem" }}>{leave.leaveType}</td><td style={{ padding: "1rem", whiteSpace: "nowrap" }}>{leave.startDate}{leave.endDate !== leave.startDate && ` – ${leave.endDate}`}</td><td style={{ padding: "1rem", color: "#64748b", maxWidth: 260 }}>{leave.reason}</td><td style={{ padding: "1rem" }}><span className="badge" style={statusStyle[leave.status]}>{leave.status === "Pending" && <Clock3 size={12} />}{leave.status}</span></td>{management && <td style={{ padding: "1rem" }}>{leave.status === "Pending" ? <div style={{ display: "flex", gap: ".5rem" }}><button className="btn" style={{ padding: ".5rem .7rem", background: "#ecfdf5", color: "#059669" }} onClick={() => review(leave, "Approved")}><Check size={14} /> Approve</button><button className="btn" style={{ padding: ".5rem .7rem", background: "#fef2f2", color: "#dc2626" }} onClick={() => review(leave, "Rejected")}><X size={14} /> Reject</button></div> : "—"}</td>}</tr>)}</tbody></table></div>}
      </div>
