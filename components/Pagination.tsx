...
export default function Pagination({ page, pageCount, pageSize, total, onPageChange, onPageSizeChange }: Props) {
  ...
  return (
    <div className="flex w-full min-w-0 items-center justify-between gap-4 border-t border-slate-200 bg-white px-3 py-2.5 max-sm:flex-col max-sm:items-stretch max-sm:gap-2">
      <span className="text-xs font-semibold text-slate-500 max-sm:text-center">Showing {first}–{last} of {total}</span>
      <div className="flex items-center gap-2 max-sm:w-full max-sm:flex-wrap max-sm:justify-center">
        ...
        <button className="grid size-9 place-items-center rounded-lg border border-slate-200 bg-slate-100 text-slate-600 transition hover:border-indigo-500 hover:bg-white hover:text-indigo-600 disabled:cursor-not-allowed disabled:opacity-40" disabled={page === 1} onClick={() => onPageChange(page - 1)} aria-label="Previous page"><ChevronLeft size={15} /></button>
        <span className="min-w-12 text-center text-xs font-bold text-slate-700">{page} / {pageCount}</span>
